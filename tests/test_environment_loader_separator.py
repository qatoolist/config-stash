...
class TestEnvironmentLoaderSeparator(unittest.TestCase):
    ...
    def test_deeply_nested_with_custom_separator(self):
        ...
        with patch.dict('os.environ', env_vars, clear=True):
            ...
            self.assertEqual(
                config['level1']['level2']['level3']['level4']['value'],
                'deep_value'
            )
            self.assertEqual(
                config['level1']['level2']['another'],
                'test'
            )
            self.assertEqual(
                config['a']['b']['c']['d']['e']['f'],
    ...
    def test_special_characters_in_separator(self):
        ...
        env_vars = {
            'PIPE_SECTION|KEY': 'value1',
            'PIPE_SECTION|NESTED|DEEP': 'value2'
        }
    ...
    def test_backward_compatibility_default_separator(self):
        ...
        env_vars = {
            'OLD_STYLE_DB__HOST': 'localhost',
            'OLD_STYLE_DB__PORT': '5432'
        }
